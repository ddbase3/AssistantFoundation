...
The current package also includes runtime/profile and provider-neutral slots that were added after the original extension audit. Detailed ownership is covered in the adjacent documents:

* runtime discovery and routing: `IAgentRuntimeRegistry`, `IAgentRuntimeSelector`, `IAgentRuntimeService`;
* runtime configuration: `IAgentConfigFormService`, `IAgentRuntimeConfigFormService`;
* explicit conversations and text tasks: `IAgentConversationService`, `IAgentConversationRuntimeService`, `IAgentTextTaskService`, `IAgentTextTaskRuntimeService`;
* context profiles: `IAgentContextProfileProvider`, `IAgentContextProfileService`;
* tool profiles and execution: `IAgentToolProfileProvider`, `IAgentToolProfileService`, `IAgentToolSet`, `IAgentConfirmableToolSet`;
* model configuration: `IAiModelConfigurationProvider`;
* file parsing: `IFileParserService`;
* retrieval schemas and diagnostics: `IRetrievalCollectionDefinition`, `IRetrievalFilterProvider`, `IRetrievalIndex`, `IRetrievalIndexInspector`, `IPhoneticEncoder`;
* speech: `ISpeechToTextService`, `IRealtimeSpeechToTextSessionService`, `ITextToSpeechService`, `ITextToSpeechStream`.

See [agent runtime contracts](agent-runtime-contracts.md), [AI model and provider contracts](ai-model-and-provider-contracts.md), [retrieval contracts](retrieval-contracts.md), and [parser and speech contracts](parser-and-speech-contracts.md).
...
See also `agent-action-approval-and-resume.md` and `agent-mutation-commit-guard.md`.
...
See `agent-capability-providers-and-modules.md`.
...
See `agent-capability-catalog-and-selection.md`.
...
See `agent-memory-and-context.md` and `agent-memory-context-profiles.md`.
...
See `agent-durable-suspensions.md`.
...
See `service-and-connection-drivers.md` and `MissionBay/docs/service-drivers.md`.
